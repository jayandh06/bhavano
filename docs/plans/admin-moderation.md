...
7. `MessagingService`: add `getOrCreateModerationThread(listingId, adminId)` (an upsert on `type: 'moderation'` — the same idempotent-get-or-create shape used for a buyer's first inquiry, see `sendFirstMessage`'s docs/plans/message-delete-and-lazy-conversation-creation.md — but keyed on `type: 'moderation'` and without the `ownerId === inquirerId` block, since here `inquirerId` is the admin, not the owner). `listConversations()`'s query and `ConversationSummaryDto` mapping pass through `type` so the web inbox can badge admin threads.
