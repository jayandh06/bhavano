...
[apps/bff/prisma/data/knownCities.json](../../apps/bff/prisma/data/knownCities.json) is the Census list of places with population 50,000 and above, including urban agglomerations. Karur is the agglomeration (2,34,191), not the smaller municipality. A census name that is already a city (Bengaluru and Bruhat Bangalore, Tiruppur and Tirupur) is stored once. [apps/bff/prisma/seedKnownCities.ts](../../apps/bff/prisma/seedKnownCities.ts) inserts each new row as `source: 'curated'`, `isPopular: false`, `catchmentKm: 25`. It does not change lat/lng or catchment on a city that already exists.
...
When two census rows share a URL slug, the higher population is kept (Aurangabad, Maharashtra over the Bihar town of the same name). A census name that is the same place under another spelling already in the file is not inserted again (Bid is Beed, Ranibennur is Ranebennuru, Durg-Bhilainagar sits inside Durg). Coordinates are taken only from a GeoNames place in the same state, so a same-name town in another state is not added. Spelling aliases added with this list: Trichy and Tiruchi → Tiruchirappalli; Hubballi → Hubli; Belagavi → Belgaum; Prayagraj → Allahabad; Thoothukudi → Tuticorin; Chhatrapati Sambhajinagar and Sambhajinagar → Aurangabad; Udhagamandalam → Ooty; Tiruppur → Tirupur; Kancheepuram → Kanchipuram; Hardwar → Haridwar; Baleshwar → Balasore; Bid → Beed; Ranibennur → Ranebennuru; Nippani → Nipani; Ropar → Rupnagar; Arni → Arani; Tirumangalam → Thirumangalam; Pilkhua → Pilkhuwa.
