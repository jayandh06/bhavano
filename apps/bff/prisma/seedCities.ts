// Needed when this file is run standalone (`tsx prisma/seedCities.ts`) rather than through
// `prisma db seed`, which loads env vars itself via `prisma.config.ts`'s own `dotenv/config`.
 ...
 * top of this (see seed.ts) doesn't have to re-fetch them. */
export async function seedCities(prisma: PrismaClient): Promise<{
  cityRecords: Map<string, string>;
  areaRecords: Map<string, string>;
}> {
  ...
  return { cityRecords, areaRecords };
}

// Runnable standalone (`tsx prisma/seedCities.ts`, wired up as the `prisma:seed:cities` package
// script) — this is what actually needs to run against production, since `prisma migrate deploy`
// only applies schema changes and never runs seeds. `seed.ts`'s own `main()` also calls
// `seedCities()` directly for local dev, so this guard just prevents this file running its own
// standalone `main` a second time when imported from there.
if (require.main === module) {
  const prisma = new PrismaClient({ adapter: new PrismaPg({ connectionString: process.env.DATABASE_URL }) });
  seedCities(prisma)
    .then(() => console.log('City/area seed complete.'))
    .catch((e) => {
      console.error(e);
      process.exitCode = 1;
    })
